...
function ActivityPage() {
  const { data, isLoading, isFetching, error, refetch } = useQuery(activityQueryOptions());
  ...
  const verifiedCount = activities.filter((a) => a.kind === "system" && a.type === "monitor_succeeded").length;
  const issueCount = activities.filter(
    (a) => a.kind === "system" && (a.type === "extraction_degraded" || a.type === "healing_started" || a.type === "healing_failed"),
  ).length;
  ...
  const filteredActivities = React.useMemo(() => {
    return activities.filter((item) => {
      ...
      if (selectedCompetitor !== "all" && item.competitorId !== selectedCompetitor) {
        return false;
      }
      ...
      if (searchQuery.trim()) {
        ...
        const compMatch = item.competitorName?.toLowerCase().includes(query) ?? false;
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Chronological stream of detected competitor campaign updates and scraping operations.
          </p>
        </div>
        ...
        >
          ...
          />
          <span>{isFetching ? "Refreshing..." : "Refresh stream"}</span>
        </Button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-xs font-medium">
            <HugeiconsIcon icon={Tag01Icon} strokeWidth={2} className="size-4 text-primary" />
        ...
        <Card className="bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-xs font-medium">
            <HugeiconsIcon icon={AiBrain01Icon} strokeWidth={2} className="size-4 text-amber-400" />
      ...
      <div className="space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 p-1 bg-muted/30 border border-border">
            <button
              type="button"
        ...
        <div className="relative">
          ...
          />
          <Input
            placeholder="Search activity by message, competitor, or event title..."
            value={searchQuery}
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed p-12 text-center bg-muted/10">
          ...
          <h3 className="text-base font-semibold text-foreground">
            {activities.length === 0 ? "No Activity Recorded Yet" : "No Matching Events Found"}
          </h3>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs text-muted-foreground px-1">
            <span>Showing {filteredActivities.length} event{filteredActivities.length === 1 ? "" : "s"}</span>
...
function ActivityCard({ item }: { item: ActivityItem }) {
  ...
  if (isCampaign) {
    ...
    const beforeVal = (item.before as Record<string, unknown>)?.value ?? item.before;
    const afterVal = (item.after as Record<string, unknown>)?.value ?? item.after;
        ...
        <CardContent className="p-4 sm:p-5 space-y-3">
          ...
          <div className="space-y-2">
            ...
            {(beforeVal != null || afterVal != null) && (
              <div className="flex items-center gap-3 p-2.5 bg-muted/20 border border-border text-xs">
                <div className="flex-1 space-y-0.5">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono">Previous</span>
                  <p className="text-muted-foreground line-through font-mono">
                    {beforeVal ? String(beforeVal) : "None"}
                  </p>
                </div>

                <HugeiconsIcon icon={ArrowRight01Icon} strokeWidth={2} className="size-3.5 text-primary shrink-0" />

                <div className="flex-1 space-y-0.5">
                  <span className="text-[10px] uppercase tracking-wider text-primary font-mono font-semibold">Detected Change</span>
          ...
          {item.metadata && typeof item.metadata === "object" && Object.keys(item.metadata).length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1.5">
              {Object.entries(item.metadata).map(([key, val]) => (
                <span
                  key={key}
                  className="inline-flex items-center gap-1 text-[10px] font-mono px-2 py-0.5 bg-muted/40 border border-border/60 text-muted-foreground"
                >
